...
async function lazyDominantColorHSL(image) {
...
async function run() {
    ...
    const _models = Object.entries(models)
        ...
        .filter((model) => model.hasOwnProperty("parent") && model.hasOwnProperty("textures"));
    ...
    await fs.writeFile("./src/assets/colors.json", JSON.stringify(colors, null, 2));
}
